...
export function calculateCurrentCaffeineLevel(historyData) {
  ...
  for (const [timestamp, entry] of Object.entries(historyData)) {
    ...
    if (Number.isNaN(parsedTimestamp) || !entry?.name) {
      continue;
    }
    const timeElapsed = currentTime - parsedTimestamp;
...
export function calculateCoffeeStats(coffeeConsumptionHistory) {
  ...
  for (const [timestamp, coffee] of Object.entries(coffeeConsumptionHistory)) {
    const parsedTimestamp = Number.parseInt(timestamp, 10);
    if (!coffee?.cost || Number.isNaN(parsedTimestamp)) {
      continue;
    }
    const date = new Date(parsedTimestamp).toISOString().split("T")[0]; // Extract date in YYYY-MM-DD format
    const caffeine = getCaffeineAmount(coffee.name);
    const cost = Number.parseFloat(coffee.cost);
...
export function calculateDailyStats(historyData) {
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    for (const [timestamp, entry] of Object.entries(historyData)) {
      const parsedTimestamp = Number.parseInt(timestamp, 10);
      if (!entry?.cost || Number.isNaN(parsedTimestamp)) {
        continue;
      }
      const entryDate = new Date(parsedTimestamp).toISOString().split("T")[0];
